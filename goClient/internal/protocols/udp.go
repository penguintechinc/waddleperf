...
type UDPTestRequest struct {
	...
	Protocol string `json:"protocol"` // raw, dns
...
func testRawUDP(target string, timeout time.Duration, result *UDPTestResult) (*UDPTestResult, error) {
	...
	if err != nil {
		...
		result.Success = true
		result.Response = "Packet sent (no response expected for raw UDP)"
	} else {
		result.Success = true
...
func testDNS(target, query string, timeout time.Duration, result *UDPTestResult) (*UDPTestResult, error) {
	...
	result.Success = true
	result.DNSResults = ips
	result.Response = fmt.Sprintf("Resolved %s to %d IPs", query, len(ips))
